...
from rest_client.conf import APIConfig, ResourceConfig, BodyParameter, QueryParameter,  ParameterConfig
from rest_client.auth import UserPassAuthConfig

class MinimalConfig(APIConfig):
    endpoint1 = ResourceConfig(path=[], method='GET')

class UserPassConfig(APIConfig):
    endpoint1 = ResourceConfig(path=[], method='GET')
...
class TestRestClient(unittest.TestCase):
    ...
    def test_login_user(self):
        ...
        rc = client.API(url=self.url, config=self.user_pass_config)
    ...
    def test_init_wrong_type(self):
        ...
        client.API(url=self.url, user=123)

    def test_init_resources(self):
        from rest_client.conf import APIConfig, ResourceConfig, BodyParameter, QueryParameter

        rc = client.API(url=self.url, config=self.minimal_config)
        self.assertEqual(rc.resources, ['endpoint1'])
        
        class TestConfig(APIConfig):
            some_function_name = ResourceConfig(path=["some", "collection", "{id}"],
                                                method="GET",
                                                parameters={
        ...
        rc = client.API(url=self.url, config=TestConfig())
