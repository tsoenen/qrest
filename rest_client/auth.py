...
class CASAuth(RESTAuthentication):
    ...
    def get_service_ticket(self):
        ...
        response = requests.post(url=tgt_url,
                                         data=body,
        ...
        if not response.ok:
            ...
            tgt_url = self.read_tgt()
            response = requests.post(url=tgt_url, data=body, verify=self.verify_ssl)
    ...
    def get_tgt_and_write(self):
        ...
        tgt_location = response.headers["location"]
        logging.debug("[CAS] TGT URI is '%'", tgt_location)
    ...
    def read_tgt(self):
        ...
        if not os.path.isfile(self.tgt_path):
            ...
            logging.debug("[CAS] TGT contents are '%s'", tgt)
            if tgt == "":
                os.remove(self.tgt_path)
                raise ValueError("[CAS] TGT file at '%s' was empty and has been removed."% tgt_path)
            else:
                return tgt
    ...
    def generate_tgt(self, overwrite=False):
        ...
        if overwrite or (not os.path.isfile(self.tgt_path)):
            logging.debug("[CAS] File '%s' does not exist or 'overwrite' is True. A new one will be created.", self.tgt_path)
            self.get_tgt_and_write()
        else:
            logging.debug("[CAS] File '%s' does exist and 'overwrite' is False. The current one will be used.", self.tgt_path)
...
class CasAuthConfig(AuthConfig):
    ...
    def __init__(self, path, service_name, granting_ticket_filepath, verify_ssl):
        ...
        self.granting_ticket_filepath = os.path.expanduser(granting_ticket_filepath)
        self.verify_ssl = verify_ssl
